...
class TestSequenceFunctions(unittest.TestCase):
    ...
    def test_search_and_visit(self):
        profiles = self.u1.search(location="10033", number=30, age_min=25,
                                  age_max=30, looking_for="straight girls only",
                                  status="single", last_online=3500)
        ...
        for p in profiles[:3]:
            ...
            self.assertEqual(p.details['last online'], 'Online now!')
    ...
    # def test_messaging(self):
        # mtext = ''.join(random.choice(string.printable) for i in range(30))
        # self.u1.message(self.u2.username, mtext)
        # self.u1.update_mailbox('outbox', pages=1)
        # m1 = self.u1.outbox[0]
        # self.u1.read(m1)
        # self.assertTrue(m1.messages[0][4:] == mtext)
